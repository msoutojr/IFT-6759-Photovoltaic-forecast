# -*- coding: utf-8 -*-
"""cnn_classfier_test.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1cPzzBZ49KrvzIx6HrqTJPYaSVHUopD7f
"""
...
datetime_test = pd.DataFrame(
    np.load(
        "/content/drive/MyDrive/asdf/photovoltaics/datetime_test.npy", allow_pickle=True
    )[()],
    columns=["d_t"],
)

pv_log_test = pd.DataFrame(
    np.load(
        "/content/drive/MyDrive/asdf/photovoltaics/pv_log_test.npy", allow_pickle=True
    )[()],
    columns=["pv_l"],
)

images_test = np.load(
    "/content/drive/MyDrive/asdf/photovoltaics/images_test.npy", allow_pickle=True
)[()]
...
for i in range(len(r)):
    if r[i] < 0.3:
        r[i] = "overcast"
    elif r[i] >= 0.3 and r[i] < 0.5:
        r[i] = "cloudy-low"
    elif r[i] >= 0.5 and r[i] < 0.7:
        r[i] = "cloudy-mid"
    elif r[i] >= 0.7 and r[i] < 0.9:
        r[i] = "cloudy-high"
    elif r[i] >= 0.9 and r[i] < 1.05:
        r[i] = "sunny"
    else:
        r[i] = "enhanced"
...
df_label.to_csv("test_label_6_classes.csv")
