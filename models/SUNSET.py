...
class SUNSET_GRU2(nn.Module):
    ...
    def __init__(self, conv_layers=2, convGRU_layers = 3, fc_layers = 2 ):
        ...
        super(SUNSET_GRU2, self).__init__()
        ...
        # FC layers
        if convGRU_layers > 0:
            dim_0 = int(hidden_dim[-1]*input_size[0]*input_size[1]*num_layers)
        else:
        ...
        self.fc2 = nn.Linear(1024, dim_reg)
        self.drop1 = nn.Dropout(self.drop_rate)
        self.drop2 = nn.Dropout(self.drop_rate)
    ...
    def forward(self, inputs=None, device="cuda:0", hidden_state=None):
        ...
        if self.convGRU_layers > 0:
                ...
                x_conc = torch.flatten(x[0][:,-1,:,:,:], start_dim=1)
                #print(x_conc.shape)
                for i in range(len(x)-1):
                    x_conc = torch.cat( (x_conc , torch.flatten(x[i+1][:,-1,:,:,:], start_dim=1)),1 )
...
class SUNSET_GRU3(nn.Module):
    ...
    def __init__(self, conv_layers=2, convGRU_layers = 3, fc_layers = 2 ):
        ...
        super(SUNSET_GRU3, self).__init__()
...
class SUNSET_LSTM(nn.Module):
    
    """
    """
    def __init__(self, conv_layers=2, convLSTM_layers = 3, fc_layers = 2 ):
        
        super(SUNSET_LSTM, self).__init__()
        ...
        if convLSTM_layers > 3:
            convLSTM_layers = 3
            print('max number of convLSTM layers = 3')
        ...
        bias = False
        return_all_layers=False
        self.LSTM = convLSTM.ConvLSTM(input_size,input_dim,hidden_dim,kernel_size,num_layers,batch_first,bias,return_all_layers).cuda()
        
        
        # FC layers
        if convLSTM_layers > 0:
            dim_0 = int(hidden_dim[-1]*input_size[0]*input_size[1])
        else:
            dim_0 = int(chanels[-1]*input_size[0]*input_size[1])
        if fc_layers == 0:
